...
l0=75; l1=150; l2=301.5; round = 2*pi;
size_ngang=30; %l0 cua~ day' duoi'
size_doc=10; %h cua~ day' duoi'
anpp = acos((abs(l0-size_ngang))/(l1+l2));
alpha = pi-anpp;
H = [0; 0; -sqrt((l1+l2)^2-(l0-size_ngang)^2)-size_doc];
H_run = [0; 0; -200]; 

%Diem~ dau` cuoi'
Pee_pre = [0; 0; -250]; %vi. tri' ban dau`
Pee_last = [-60; -50; -310]; %vi. tri' muon' den'

Hs_pre = (Pee_pre - H_run)/100; 
Hs_last = (H_run - Pee_last)/100;
...
the4_3_1 = Fun001_BasicAtan2( (2*l1*l2*cos(the5_3(1,1))), (0) , (z3^2+(l0+l2*sqrt(3)*sin(the5_3(1,1))+2*x2)^2-l1^2-(l2*cos(the5_3(1,1)))^2) );
the4_3_2 = Fun001_BasicAtan2( (2*l1*l2*cos(the5_3(2,1))), (0) , (z3^2+(l0+l2*sqrt(3)*sin(the5_3(2,1))+2*x2)^2-l1^2-(l2*cos(the5_3(2,1)))^2) );
